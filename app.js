...
app.get('/', (req, res) => {
  axios.get('https://api.coinmarketcap.com/v1/ticker/?limit=400').then((response) => {
    ...
    function isBTC(currency) {
      return currency.symbol === 'BTC';
    }
    function isMIOTA(currency) {
      return currency.symbol === 'MIOTA';
    }
    function isPOWR(currency) {
      return currency.symbol === 'POWR';
    }
    function isARDR(currency) {
      return currency.symbol === 'ARDR';
    }
    const btcObj = currencies.find(isBTC);
    const miotaObj = currencies.find(isMIOTA);
    const powrObj = currencies.find(isPOWR);
    const ardrObj = currencies.find(isARDR);
    res.render('home.hbs', {
      hold: "active",
      interest: "not-active",
...
//     prices.pu_dedeTC = response.data[0].price_usd;
